System.register(["__unresolved_0", "fs", "url", "child_process", "http", "https"], function (_export, _context) {
  "use strict";
  ...
  return {
    setters: [function (_unresolved_) {
      ...
      _req0 = _url.__cjsMetaURL;
    }, function (_child_process) {
      _req1 = _child_process.__cjsMetaURL;
    }, function (_http) {
      _req2 = _http.__cjsMetaURL;
    }, function (_https) {
        ...
        function XMLHttpRequest(opts) {
          ...
          this.send = function (data) {
            ...
            if (settings.async) {
              ...
              var responseHandler = function responseHandler(resp) {
                ...
                response.on('end', function () {
                  if (sendFlag) {
                    ...
                    setState(self.DONE); // Construct responseText from response
              ...
              var execString = "var http = require('http'), https = require('https'), fs = require('fs');" + "var doRequest = http" + (ssl ? "s" : "") + ".request;" + "var options = " + JSON.stringify(options) + ";" + "var responseText = '';" + "var responseData = Buffer.alloc(0);" + "var req = doRequest(options, function(response) {" + "response.on('data', function(chunk) {" + "  var data = Buffer.from(chunk);" + "  responseText += data.toString('utf8');" + "  responseData = Buffer.concat([responseData, data]);" + "});" + "response.on('end', function() {" + "fs.writeFileSync('" + contentFile + "', JSON.stringify({err: null, data: {statusCode: response.statusCode, headers: response.headers, text: responseText, data: responseData.toString('base64')}}), 'utf8');" + "fs.unlinkSync('" + syncFile + "');" + "});" + "response.on('error', function(error) {" + "fs.writeFileSync('" + contentFile + "', 'NODE-XMLHTTPREQUEST-ERROR:' + JSON.stringify(error), 'utf8');" + "fs.unlinkSync('" + syncFile + "');" + "});" + "}).on('error', function(error) {" + "fs.writeFileSync('" + contentFile + "', 'NODE-XMLHTTPREQUEST-ERROR:' + JSON.stringify(error), 'utf8');" + "fs.unlinkSync('" + syncFile + "');" + "});" + (data ? "req.write('" + JSON.stringify(data).slice(1, -1).replace(/'/g, "\\'") + "');" : "") + "req.end();"; // Start the other Node Process, executing this string
          ...
          this.dispatchEvent = function (event) {
            ...
            if (event in listeners) {
              var _loop = function _loop(i) {
                if (_this.readyState === _this.DONE) setTimeout(function () {
...
//# sourceMappingURL=f09159b917545b42178fe2ad5fee1e16d7ce5bb6.js.map