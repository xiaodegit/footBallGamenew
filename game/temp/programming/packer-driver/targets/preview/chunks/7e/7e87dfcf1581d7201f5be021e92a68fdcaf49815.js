    ...
    execute: function () {
      ...
      _export("onTouchStick", onTouchStick = (_dec = ccclass('onTouchStick'), _dec2 = property(Node), _dec(_class = (_class2 = class onTouchStick extends Component {
        ...
        setPositionToTouch(event, targetNode) {
          ...
          // 获取手指触摸位置的 UI 坐标
          var touchLocation = event.getUILocation(); // 将 UI 坐标转换为世界坐标

          var uiPoint = new Vec3(touchLocation.x, touchLocation.y, 0); // 将世界坐标转换为操纵杆父节点的局部坐标

          var parentUITransform = (_targetNode$parent = targetNode.parent) == null ? void 0 : _targetNode$parent.getComponent(UITransform);

          if (!parentUITransform) {
            return;
          }

          var localPosition = parentUITransform.convertToNodeSpaceAR(uiPoint); // 更新节点的局部位置

          targetNode.setPosition(localPosition);
        }

        updateStickMove() {
          var handle = this.node.children[0].children[0];
          if (!handle) return; // 获取手柄的当前局部坐标

          this.currentPosition = handle.position; // 限制 X 轴的范围在 -100 到 100 之间

          var maxX = 100;
          var minX = -100; // 限制 X 轴的位置

          var clampedX = Math.min(Math.max(this.currentPosition.x, minX), maxX); // Y 轴保持不变（如果需要固定在 0，可以将 currentPosition.y 设置为 0）

          var fixedY = 0; // 更新手柄的位置，只改变 X 轴，保持 Y 和 Z 轴不变
        ...
        singleTouch(event) {
          // 获取当前屏幕上所有触摸点
          var touches = event.getTouches(); // 只有当触摸点数量为1时，才标识为单点触摸
